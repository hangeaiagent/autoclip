...
const HomePage: React.FC = () => {
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      message.error(t('home.loadProjectsFailed'))
  ...
  const handleStartProcessing = async (projectId: string) => {
    try {
      ...
      message.success(t('home.processingStarted'))
      // 立即刷新项目列表以显示最新状态
      setTimeout(async () => {
        try {
          await refreshNow()
      ...
      const errorMessage = (error as { userMessage?: string })?.userMessage || t('home.startProcessingFailed')
      ...
      if ((error as { code?: string; message?: string })?.code === 'ECONNABORTED' || (error as { code?: string; message?: string })?.message?.includes('timeout')) {
        message.info(t('home.requestTimeout'), 5)
        // 延迟刷新项目列表
        setTimeout(async () => {
          try {
            await refreshNow()
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div style={{
                  ...
                  <Text style={{ color: '#4facfe', fontWeight: 600, fontSize: '14px' }}>
                    {t('home.totalProjects', { count: filteredProjects.length })}
                  </Text>
                </div>
              </div>
              ...
              <div style={{ 
                ...
                <Select
                  ...
                  allowClear
                >
                  <Option value="all" style={{ color: '#ffffff' }}>{t('home.allStatus')}</Option>
                  ...
                  <Option value="error" style={{ color: '#ff4d4f' }}>{t('home.failed')}</Option>
                </Select>
              </div>
            </div>
                 ...
                 <div style={{
                   ...
                   <Empty
                     ...
                     description={
                       <div>
                         <Text type="secondary">
                           {projects.length === 0 ? t('home.noProjectsYet') : t('home.noMatchingProjects')}
                         </Text>
                       </div>
                     }
                   />
