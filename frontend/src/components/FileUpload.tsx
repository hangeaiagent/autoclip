...
const FileUpload: React.FC<FileUploadProps> = ({ onUploadSuccess }) => {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      ...
      try {
        ...
        message.error(t('fileUpload.loadCategoriesFailed'))
      } finally {
  ...
  const handleUpload = async () => {
    if (!files.video) {
      message.error(t('fileUpload.selectVideoFile'))
      return
    }

    if (!projectName.trim()) {
      message.error(t('fileUpload.enterProjectName'))
      return
    }
    ...
    try {
      ...
      message.success(t('fileUpload.projectCreated'))
      ...
      if (error.response?.status === 413) {
        errorMessage = t('fileUpload.fileTooLarge')
        errorType = 'warning'
      } else if (error.response?.status === 415) {
        errorMessage = t('fileUpload.unsupportedFormat')
        errorType = 'warning'
      } else if (error.response?.status === 400) {
        if (error.response?.data?.detail) {
          errorMessage = error.response.data.detail
        } else {
          errorMessage = t('fileUpload.fileFormatIssue')
        }
      } else if (error.response?.status === 500) {
        errorMessage = t('fileUpload.serverError')
      } else if (error.code === 'ECONNABORTED') {
        errorMessage = t('fileUpload.uploadTimeout')
      ...
      if (error.code === 'ECONNABORTED' || error.response?.status >= 500) {
        message.info(t('fileUpload.persistentIssue'), 5)
      }
    } finally {
      setUploading(false)
    }
    ...
    <div style={{
      ...
      >
        ...
        <div>
          <Text strong style={{ 
            ...
            {isDragActive ? t('fileUpload.dropToImport') : t('fileUpload.clickOrDrag')}
          </Text>
          <Text style={{ color: '#cccccc', fontSize: '14px', lineHeight: '1.5' }}>
            {t('fileUpload.supportedFormats')}<Text style={{ color: '#52c41a', fontWeight: 600 }}>{t('fileUpload.optionalSubtitle')}</Text>
          </Text>
        </div>
      </div>
      ...
      {files.video && (
        <div style={{ marginBottom: '16px' }}>
          <Text strong style={{ color: '#ffffff', fontSize: '14px', marginBottom: '8px', display: 'block' }}>
            {t('fileUpload.projectName')}
          </Text>
          <Input
            value={projectName}
            onChange={(e) => setProjectName(e.target.value)}
            placeholder={t('fileUpload.projectNamePlaceholder')}
            style={{ 
              height: '40px',
              borderRadius: '12px',
              fontSize: '14px',
      ...
      {uploading && (
        <div style={{ 
          ...
          <div style={{ marginBottom: '12px' }}>
            <Text style={{ color: '#ffffff', fontWeight: 600, fontSize: '14px' }}>{t('fileUpload.importProgress')}</Text>
      ...
      {files.video && (
        <div style={{ textAlign: 'center', marginTop: '8px' }}>
          ...
          >
            {uploading ? t('fileUpload.importing') : t('fileUpload.startImportProcess')}
          </Button>
        </div>
      )}
    </div>
